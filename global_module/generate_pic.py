...
def load_dataset(Dataset):
    if Dataset == 'IN':
        ...
        TOTAL_SIZE = 10249
        VALIDATION_SPLIT = Value.VALIDATION_SPLIT_IN
    ...
    if Dataset == 'UP':
        ...
        TOTAL_SIZE = 42776
        VALIDATION_SPLIT = Value.VALIDATION_SPLIT_UP
        TRAIN_SIZE = math.ceil(TOTAL_SIZE * VALIDATION_SPLIT)
    ...
    if Dataset == 'KSC':
        ...
        TOTAL_SIZE = 5211
        VALIDATION_SPLIT = Value.VALIDATION_SPLIT_KSN
        TRAIN_SIZE = math.ceil(TOTAL_SIZE * VALIDATION_SPLIT)
...
def generate_png(all_iter, net, gt_hsi, Dataset, device, total_indices,path):
    ...
    for X, y in all_iter:
        ...
        net.eval()  # 评估模式, 这会关闭dropout
        # print(net(X))
        pred_test.extend(np.array(net(X).cpu().detach().numpy().argmax(axis=1)))
        # pred_test.extend(np.array(net(X).cpu().argmax(axis=1)))
    ...
    classification_map(gt_re, gt_hsi, 300,
                       '../all_classification_maps/' + Dataset + '_gt.png')
